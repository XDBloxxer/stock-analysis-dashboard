...
FIX v3.3:
  - render_price_journey: marker color list contained None values when a
    snapshot had no close price, causing a Plotly ValueError. Fixed by
    converting None -> float('nan') and pre-computing discrete hex colors
    as a plain list instead of using a colorscale mapping.
"""
...
def render_price_journey(symbol: str, open_df, close_df, prior_open_df, prior_close_df):
    ...
    # FIX v3.3: Convert None -> nan so Plotly renders gaps cleanly, and
    # pre-compute a plain list of hex color strings (Plotly rejects None
    # values inside a color array, which caused the ValueError crash).
    safe_closes = [float('nan') if c is None else c for c in closes]
    valid_vals  = [c for c in safe_closes if not math.isnan(c)]
    c_min   = min(valid_vals) if valid_vals else 0.0
    c_max   = max(valid_vals) if valid_vals else 1.0
    c_range = c_max - c_min if c_max != c_min else 1.0

    def _close_color(c):
        if math.isnan(c):
            return COLORS['primary']
        ratio = (c - c_min) / c_range
        if ratio < 0.33:
            return COLORS['red']
        elif ratio < 0.66:
            return COLORS['primary']
    ...
    marker_colors = [_close_color(c) for c in safe_closes]

    fig.add_trace(go.Scatter(
        x=labels, y=safe_closes,
        mode='lines+markers',
        name='Close Price',
